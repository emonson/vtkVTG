/*=========================================================================
  ...
  Module:    $RCSfile: vtkMyPolyVertex.cxx,v $
...
vtkCxxRevisionMacro(vtkMyPolyVertex, "$Revision: 1.2 $");
vtkStandardNewMacro(vtkMyPolyVertex);

//----------------------------------------------------------------------------
vtkMyPolyVertex::vtkMyPolyVertex()
{
...
//----------------------------------------------------------------------------
int vtkMyPolyVertex::EvaluatePosition(double x[3], double* closestPoint,
...
//----------------------------------------------------------------------------
void vtkMyPolyVertex::EvaluateLocation(int& subId, 
...
//----------------------------------------------------------------------------
int vtkMyPolyVertex::CellBoundary(int subId, double pcoords[3], vtkIdList *pts)
{
...
//----------------------------------------------------------------------------
void vtkMyPolyVertex::Contour(double value, vtkDataArray *cellScalars, 
...
//
int vtkMyPolyVertex::IntersectWithLine(double p1[3], double p2[3], 
...
//----------------------------------------------------------------------------
int vtkMyPolyVertex::Triangulate(int vtkNotUsed(index), vtkIdList *ptIds, 
                               vtkPoints *pts)
{
  int subId;
...
//----------------------------------------------------------------------------
void vtkMyPolyVertex::Derivatives(int vtkNotUsed(subId), 
                                double vtkNotUsed(pcoords)[3], 
...
//----------------------------------------------------------------------------
void vtkMyPolyVertex::Clip(double value, vtkDataArray *cellScalars, 
                         vtkPointLocator *locator, vtkCellArray *verts,
                         vtkPointData *inPd, vtkPointData *outPd,
                         vtkCellData *inCd, vtkIdType cellId,
                         vtkCellData *outCd, int insideOut)
{
  ...
  for ( i=0; i < numPts; i++ )
    ...
    if ( (!insideOut && s > value) || (insideOut && s <= value) )
      {
      this->Points->GetPoint(i,x);
      // MODIFIED to allow insertion of duplicate points (don't merge coincident points)
...
int vtkMyPolyVertex::GetParametricCenter(double pcoords[3])
{
...
//----------------------------------------------------------------------------
void vtkMyPolyVertex::InterpolateFunctions(double pcoords[3], double *weights)
{
  (void)pcoords;
  (void)weights;
}
...
//----------------------------------------------------------------------------
void vtkMyPolyVertex::InterpolateDerivs(double pcoords[3], double *derivs)
{
  (void)pcoords;
  (void)derivs;
}
...
//----------------------------------------------------------------------------
void vtkMyPolyVertex::PrintSelf(ostream& os, vtkIndent indent)
{
