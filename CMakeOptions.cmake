...
#
SET(LIBRARY_OUTPUT_PATH ${VTKVTG_BINARY_DIR}/bin CACHE PATH
    "Single output directory for building all libraries.")
SET(EXECUTABLE_OUTPUT_PATH ${VTKVTG_BINARY_DIR}/bin CACHE PATH
...
# Copy the CMake option to a setting with VTKVTG_ prefix for use in
# our project.  This name is used in vtkvtgConfigure.h.in.
SET(VTKVTG_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS})
...
IF (VTK_WRAP_TCL)
  ...
  OPTION(VTKVTG_WRAP_TCL
         "Wrap classes into the TCL interpreted language."
         ON)

  IF(VTKVTG_WRAP_TCL)
    INCLUDE(${VTK_CMAKE_DIR}/vtkWrapTcl.cmake)
  ENDIF(VTKVTG_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (VTKVTG_WRAP_TCL)
    MESSAGE("Warning. VTKVTG_WRAP_TCL is ON but the VTK version you have "
            "chosen has not support for Tcl (VTK_WRAP_TCL is OFF).  "
            "Please set VTKVTG_WRAP_TCL to OFF.")
    SET (VTKVTG_WRAP_TCL OFF)
  ENDIF (VTKVTG_WRAP_TCL)
...
IF (VTK_WRAP_PYTHON)
  ...
  IF (VTKVTG_WRAP_PYTHON)
    INCLUDE(${VTK_CMAKE_DIR}/vtkWrapPython.cmake)
    IF (WIN32)
      IF (NOT BUILD_SHARED_LIBS)
        MESSAGE(FATAL_ERROR "Python support requires BUILD_SHARED_LIBS to be ON.")
        SET (VTKVTG_CAN_BUILD 0)
      ENDIF (NOT BUILD_SHARED_LIBS)
    ENDIF (WIN32)
  ENDIF (VTKVTG_WRAP_PYTHON)

ELSE (VTK_WRAP_PYTHON)

  IF (VTKVTG_WRAP_PYTHON)
    MESSAGE("Warning. VTKVTG_WRAP_PYTHON is ON but the VTK version you have "
            "chosen has not support for Python (VTK_WRAP_PYTHON is OFF).  "
            "Please set VTKVTG_WRAP_PYTHON to OFF.")
    SET (VTKVTG_WRAP_PYTHON OFF)
  ENDIF (VTKVTG_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  OPTION(VTKVTG_WRAP_JAVA
         "Wrap classes into the Java interpreted language."
         ON)

  IF (VTKVTG_WRAP_JAVA)
    SET(VTK_WRAP_JAVA3_INIT_DIR "${VTKVTG_SOURCE_DIR}/Wrapping")
    ...
    SET(VTK_JAVA_HOME ${VTKVTG_BINARY_DIR}/java/vtkvtg)
    MAKE_DIRECTORY(${VTK_JAVA_HOME})
  ENDIF (VTKVTG_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (VTKVTG_WRAP_JAVA)
    MESSAGE("Warning. VTKVTG_WRAP_JAVA is ON but the VTK version you have "
            "chosen has not support for Java (VTK_WRAP_JAVA is OFF).  "
            "Please set VTKVTG_WRAP_JAVA to OFF.")
    SET (VTKVTG_WRAP_JAVA OFF)
  ENDIF (VTKVTG_WRAP_JAVA)

ENDIF (VTK_WRAP_JAVA)

# Setup our local hints file in case wrappers need them.
SET(VTK_WRAP_HINTS ${VTKVTG_SOURCE_DIR}/Wrapping/hints)
