...
bool vtkGraphItem::Paint(vtkContext2D *painter)
{
  ...
  while (it->HasNext())
    ...
    this->Impl->GetPosition(e.Target, line+2);
		line[0] += (this->imWidth*this->imScale)/2.0f;
		line[2] += (this->imWidth*this->imScale)/2.0f;
		line[1] += (this->imHeight*this->imScale)/2.0f;
		line[3] += (this->imHeight*this->imScale)/2.0f;
    painter->DrawLine(line);
    }

  float dims[4] = {0.0f, 0.0f, 
  								this->imWidth*this->imScale, 
  								this->imHeight*this->imScale};
...
bool vtkGraphItem::Hit(const vtkContextMouseEvent &mouse)
{
  ...
  for (vtkIdType i = this->Graph->GetNumberOfVertices()-1; i >= 0; --i)
    ...
    if (mouse.Pos[0] > pos[0] &&
        mouse.Pos[0] < pos[0]+this->imWidth*this->imScale &&
        mouse.Pos[1] > pos[1] &&
        mouse.Pos[1] < pos[1]+this->imHeight*this->imScale)
      {
      this->HitVertex = i;
      return true;
      }
...
void vtkGraphItem::UpdatePositions()
{
  ...
  float imW = this->imWidth*this->imScale*1.5f;
  float imH = this->imHeight*this->imScale*1.5f;
  float restDistance = (imW > imH) ? imW : imH;
