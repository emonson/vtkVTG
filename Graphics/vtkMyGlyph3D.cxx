/*=========================================================================
  ...
  Module:    $RCSfile: vtkMyGlyph3D.cxx,v $
...
vtkCxxRevisionMacro(vtkMyGlyph3D, "$Revision: 1.125 $");
vtkStandardNewMacro(vtkMyGlyph3D);
  ...
  for (inPtId=0; inPtId < numPts; inPtId++)
    ...
    if ( this->Scaling )
      ...
      if ( this->ScaleMode == VTK_DATA_SCALING_OFF )
        {
        scalex = this->ScaleXfactor;
        scaley = scalez = this->ScaleYZfactor;
        }
      else
        {
        scalex *= this->ScaleXfactor;
        scaley *= this->ScaleYZfactor;
        scalez *= this->ScaleYZfactor;
        }
...
void vtkMyGlyph3D::SetSourceConnection(int id, vtkAlgorithmOutput* algOutput)
{
  if (id < 0)
    {
...
//----------------------------------------------------------------------------
// Specify a source object at a specified table location.
void vtkMyGlyph3D::SetSource(int id, vtkPolyData *pd)
{
  if (id < 0)
    {
    vtkErrorMacro("Bad index " << id << " for source.");
...
vtkPolyData *vtkMyGlyph3D::GetSource(int id)
{
...
//----------------------------------------------------------------------------
void vtkMyGlyph3D::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Scale X Factor: " << this->ScaleXfactor << "\n";
  os << indent << "Scale YZ Factor: " << this->ScaleYZfactor << "\n";
...
int vtkMyGlyph3D::RequestUpdateExtent(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **inputVector,
  vtkInformationVector *outputVector)
{
...
//----------------------------------------------------------------------------
vtkPolyData* vtkMyGlyph3D::GetSource(int idx, vtkInformationVector *sourceInfo)
{
...
//----------------------------------------------------------------------------
int vtkMyGlyph3D::FillInputPortInformation(int port, vtkInformation *info)
{
  if (port == 0)
    {
