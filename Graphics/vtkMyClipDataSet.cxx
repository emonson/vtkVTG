/*=========================================================================
  ...
  Module:    $RCSfile: vtkMyClipDataSet.cxx,v $
...
vtkCxxRevisionMacro(vtkMyClipDataSet, "$Revision: 1.54 $");
vtkStandardNewMacro(vtkMyClipDataSet);
vtkCxxSetObjectMacro(vtkMyClipDataSet,ClipFunction,vtkImplicitFunction);

//----------------------------------------------------------------------------
// Construct with user-specified implicit function; InsideOut turned off; value
// set to 0.0; and generate clip scalars turned off.
vtkMyClipDataSet::vtkMyClipDataSet(vtkImplicitFunction *cf)
{
  ...
  this->InternalProgressObserver->SetCallback(
    &vtkMyClipDataSet::InternalProgressCallbackFunction);
...
//----------------------------------------------------------------------------
vtkMyClipDataSet::~vtkMyClipDataSet()
{
  if ( this->Locator )
    {
...
//----------------------------------------------------------------------------
void vtkMyClipDataSet::InternalProgressCallbackFunction(vtkObject* arg,
                                                      unsigned long,
                                                      void* clientdata,
                                                      void*)
{
  reinterpret_cast<vtkMyClipDataSet*>(clientdata)
    ->InternalProgressCallback(static_cast<vtkAlgorithm *>(arg));
}

//----------------------------------------------------------------------------
void vtkMyClipDataSet::InternalProgressCallback(vtkAlgorithm *algorithm)
{
...
unsigned long vtkMyClipDataSet::GetMTime()
{
...
vtkUnstructuredGrid *vtkMyClipDataSet::GetClippedOutput()
{
...
//
int vtkMyClipDataSet::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  // MODIFIED: Used to be (numCells < 1)
  if ( numCells < 1 )
    {
  ...
  for (vtkIdType cellId=0; cellId < numCells && !abort; cellId++)
	...
	// MODIFIED: Want to avoid removing coincident points in PolyVertex type
	//  so trying to cast VTK_POLY_VERTEX to vtkMyPolyVertex for modified clipping routine
	if ( cell->GetCellType() == VTK_POLY_VERTEX )
	  {
	  double s, x[3];
	  int i, newCellId, numPts=cell->Points->GetNumberOfPoints();
	  vtkIdType pts[1];
	  ...
	  for ( i=0; i < numPts; i++ )
		...
		if ( (!this->InsideOut && s > value) || (this->InsideOut && s <= value) )
		  {
		  cell->Points->GetPoint(i,x);
		  pts[0] = this->Locator->InsertNextPoint(x);
		  outPD->CopyData(inPD,cell->PointIds->GetId(i),pts[0]);
		  newCellId = conn[0]->InsertNextCell(1,pts);
...
//----------------------------------------------------------------------------
int vtkMyClipDataSet::ClipPoints(vtkDataSet* input, 
...
void vtkMyClipDataSet::SetLocator(vtkPointLocator *locator)
{
...
//----------------------------------------------------------------------------
void vtkMyClipDataSet::CreateDefaultLocator()
{
  if ( this->Locator == NULL )
    {
...
//----------------------------------------------------------------------------
void vtkMyClipDataSet::ClipVolume(vtkDataSet *input, vtkUnstructuredGrid *output)
{
...
//----------------------------------------------------------------------------
int vtkMyClipDataSet::FillInputPortInformation(int, vtkInformation *info)
{
...
//----------------------------------------------------------------------------
void vtkMyClipDataSet::PrintSelf(ostream& os, vtkIndent indent)
{
...
int vtkMyClipDataSet::ProcessRequest(vtkInformation* request,
                                   vtkInformationVector** inputVector,
                                   vtkInformationVector* outputVector)
{
